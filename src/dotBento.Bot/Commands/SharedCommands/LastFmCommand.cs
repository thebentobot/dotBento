    ...
    SpotifyApiService spotifyApiService)
{
    ...
    public async Task<ResponseModel> GetTopArtists(long userId, string discordName, string discordAvatarUrl, string period)
    {
        ...
        var topArtistsResponse = await lastFmCommands.GetTopArtists(lastFmUser.Value.Lastfm1, config.Value.LastFmApiKey, period);
        ...
        foreach (var artistChunk in artistPageChunks)
        {
            ...
            var thumbnailUrl = artistResult.IsSuccess && artistResult.Value.Images.Any() ? 
                               artistResult.Value.Images.First().Url : discordAvatarUrl;
    ...
    public async Task<ResponseModel> GetTopAlbums(long userId, string discordName, string discordAvatarUrl, string period)
    {
        var embed = new ResponseModel
        {
            ResponseType = ResponseType.Paginator,
        ...
        var topAlbumsResponse = await lastFmCommands.GetTopAlbums(lastFmUser.Value.Lastfm1, config.Value.LastFmApiKey, period);
    ...
    public async Task<ResponseModel> GetTopTracks(long userId, string discordName, string discordAvatarUrl, string period)
    {
        var embed = new ResponseModel
        {
            ResponseType = ResponseType.Paginator,
        ...
        var topTracksResponse = await lastFmCommands.GetTopTracks(lastFmUser.Value.Lastfm1, config.Value.LastFmApiKey, period);
    ...
    public async Task<ResponseModel> GetRecentTracks(long userId, string username, string userAvatar)
    {
        ...
        var recentTracksResponse = await lastFmCommands.GetRecentTracks(lastFmUser.Value.Lastfm1, config.Value.LastFmApiKey);
