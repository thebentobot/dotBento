...
public sealed class AvatarSlashCommand(InteractiveService interactiveService, AvatarCommand avatarCommand, UserSettingService userSettingService, GuildMemberLookupService memberLookup) : ApplicationCommandModule<ApplicationCommandContext>
{
    ...
    )
    {
        ...
        var guildUser = await memberLookup.GetOrFetchAsync(Context.Guild!.Id, targetUserId, Context.Guild);
        if (guildUser is null)
        {
            var err = new dotBento.Bot.Models.NetCord.ResponseModel { ResponseType = dotBento.Bot.Enums.ResponseType.Embed };
        ...
        await Context.SendResponse(interactiveService, await avatarCommand.ServerAvatarCommand(guildUser), hide ?? await userSettingService.ShouldHideCommandsAsync((long)Context.User.Id));
    }
}
